...
class CNN_LSTM_HR_Dataset(Dataset):
    ...
    def _prepare_dataset(self):
        ...
        for timestamp in valid_ts:
            try:
                # Input time windows (past 10)
                input_times = [timestamp - pd.Timedelta(minutes=delta) for delta in [120, 105, 90, 75, 60, 45, 30, 15, 0]]
                input_slices = self.compiled_df.loc[input_times]
                if isinstance(input_slices, pd.Series):
                    input_slices = input_slices.to_frame().T  # convert single row to DataFrame
                elif isinstance(input_slices, list):
                    input_slices = pd.concat(input_slices, axis=0)
                input_images = tabular_to_image(input_slices, self.pixel_coords, self.image_shape)  # (9, H, W, C)
    ...
    def get_rejected_samples(self):
        ...
        # If currently (N, C, T, H, W), convert to (N, T, C, H, W)
        #if input_tensor.shape[1] == self.image_shape[0]:  # Heuristic: check if 2nd dim is height
...
class CNN_LSTM_HR_Module(nn.Module):
    def __init__(self, num_channels=5, num_future_steps=5, hidden_size=256):
        super(CNN_LSTM_HR_Module, self).__init__()
    ...
    def forward(self, x):
        # x: (batch_size, seq_len=9, channels=5, height, width)
